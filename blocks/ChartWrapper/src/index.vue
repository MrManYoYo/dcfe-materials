        ...
        <Row>
          <Col span="12">
            <FormItem label="" prop="startTime">
              <DatePicker v-model="searchForm.startTime" transfer format="yyyy年MM月dd日"
                type="date"
                :options="startDateOptions"
                class="search-form-datepicker" placeholder="请选择开始时间" />
            </FormItem>
          </Col>
          <Col span="12">
            <FormItem label="" prop="endTime">
              <DatePicker v-model="searchForm.endTime" transfer format="yyyy年MM月dd日"
                type="date"
                :options="endDateOptions" class="search-form-datepicker" placeholder="请选择结束时间" />
            </FormItem>
          </Col>
        </Row>
      </FormItem>
  ...
  computed: {
      ...
      return {
        disabledDate (date) {
          ...
          if (_this.searchForm.endTime) {
            return date && date.valueOf() > new Date(_this.searchForm.endTime).valueOf() - 1000 * 60 * 60 * 24
          } else {
            return false
          }
        }
    ...
    endDateOptions () {
      ...
      return {
        disabledDate (date) {
          // 存在开始日期
          if (_this.searchForm.startTime) {
            return date && date.valueOf() < new Date(_this.searchForm.startTime).valueOf() + 1000 * 60 * 60 * 24
          } else {
            return false
          }
        }
